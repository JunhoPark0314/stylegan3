...
def calc_mag(source, interp, device=torch.device('cuda')):
	...
	torch.multiprocessing.set_start_method('spawn')
	dest = "Network_magnitude"
	baseline_network = "baseline" in source
	conv0_weight = lambda x, k : x.conv0.weight * x.conv0.weight_gain if baseline_network else x.conv0_weight(device, k)
	conv1_weight = lambda x, k : x.conv1.weight * x.conv1.weight_gain if baseline_network else x.conv1_weight(device, k)
	...
	for res in D.block_resolutions:
		block = getattr(D, f'b{res}')
		res_conv0_weight = conv0_weight(block, 3)
		res_conv1_weight = conv1_weight(block, 3)

		res_conv0_weight = res_conv0_weight.flatten(0,1).flatten(1,2)
		res_conv1_weight = res_conv1_weight.flatten(0,1).flatten(1,2)

		cov_0 = (res_conv0_weight.T @ res_conv0_weight)
		cov_1 = (res_conv1_weight.T @ res_conv1_weight)

		save_image(cov_0.abs().log() / cov_0.abs().log().max(), f'b{res}_conv0.png')
		save_image(cov_1.abs().log() / cov_1.abs().log().max(), f'b{res}_conv1.png')
