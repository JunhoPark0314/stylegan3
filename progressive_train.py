...
def subprocess_fn(rank, c, temp_dir, loop_module):
    ...
    dnnlib.util.call_func_by_name(rank=rank, **c, func_name=loop_module)

#----------------------------------------------------------------------------

def launch_training(c, desc, outdir, dry_run, loop_module):
...
def main(data, **kwargs):
    ...
    if opts.cfg == 'stylegan2':
    ...
    elif opts.cfg == 'stylegan3-v1':
        c.G_kwargs.class_name = 'training.networks_stylegan3_ver1.Generator'
        c.D_kwargs.class_name = 'training.networks_stylegan3_ver1.Discriminator'
        c.G_kwargs.magnitude_ema_beta = 0.5 ** (c.batch_size / (20 * 1e3))  
        c.D_kwargs.four_kwargs = dnnlib.EasyDict(freq_dim=64, band_scale=8, kernel_size=3)
    elif opts.cfg == 'stylegan3-v2':
        c.G_kwargs.class_name = 'training.networks_stylegan3_ver2.Generator'
        c.D_kwargs.class_name = 'training.networks_stylegan3_ver2.Discriminator'
        c.G_kwargs.magnitude_ema_beta = 0.5 ** (c.batch_size / (20 * 1e3))  
    elif opts.cfg == 'stylegan3-v3':
        c.G_kwargs.class_name = 'training.networks_stylegan3_ver3.Generator'
        c.D_kwargs.class_name = 'training.networks_stylegan3_ver3.Discriminator'
        c.G_kwargs.magnitude_ema_beta = 0.5 ** (c.batch_size / (20 * 1e3))  
    elif opts.cfg == 'stylegan3-v4':
        c.G_kwargs.class_name = 'training.networks_stylegan3_ver4.Generator'
        c.D_kwargs.class_name = 'training.networks_stylegan3_ver4.Discriminator'
        c.G_kwargs.magnitude_ema_beta = 0.5 ** (c.batch_size / (20 * 1e3))  
        c.D_kwargs.four_kwargs = dnnlib.EasyDict(freq_dim=64, band_scale=24)
        c.G_kwargs.freq_dim=64
        c.G_kwargs.band_scale=24
        c.G_kwargs.output_scale=1
        ...
        c.loss_kwargs.class_name = 'training.progressive_loss.StyleGAN2Loss'
        c.G_kwargs.magnitude_ema_beta = 0.5 ** (c.batch_size / (20 * 1e3))  
        ...
        if opts.cfg == 'stylegan3-sn':
            c.G_kwargs.class_name = 'training.networks_stylegan3_sn.Generator'
    ...
    # Description string.
    desc = f'{opts.cfg:s}-{dataset_name:s}-gpus{c.num_gpus:d}-batch{c.batch_size:d}-gamma{c.loss_kwargs.r1_gamma:g}-loop{opts.loop_module}'
    if opts.desc is not None:
        desc += f'-{opts.desc}'
    
    loop_module = "training.{}.training_loop".format(opts.loop_module)

    # Launch.
    launch_training(c=c, desc=desc, outdir=opts.outdir, dry_run=opts.dry_run, loop_module=loop_module)
