...
# from op import FusedLeakyReLU, fused_leaky_relu, upfirdn2d_
...
class EqualLinear(nn.Module):
	...
	def forward(self, input):
		if self.activation:
			...
			out = bias_act.bias_act(out, self.bias * self.lr_mul, act="lrelu", alpha=0.2)
...
class StyledConv(nn.Module):
	...
	def forward(self, input, style, noise=None):
		...
		if self.activate == 'lrelu':
			out = bias_act.bias_act(out, act='lrelu', gain=np.sqrt(0.5), alpha=0.2)
		else:
			out = self.activate(out)
