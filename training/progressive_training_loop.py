...
def training_loop(
    ...
    training_set_dict = {}
    training_set_sampler_dict = {}
    training_set_iterator_dict = {}
    batch_size_per_key = {}
    ...
    for k in training_set_kwargs.keys():
        ...
        training_set_sampler_dict[k] = misc.InfiniteSampler(dataset=training_set_dict[k], rank=rank, num_replicas=num_gpus, seed=random_seed)
        batch_size_per_key[k] = curr_batch_size
        training_set_iterator_dict[k] = iter(torch.utils.data.DataLoader(dataset=training_set_dict[k], sampler=training_set_sampler_dict[k], batch_size=batch_size_per_key[k]//num_gpus, **data_loader_kwargs))
        curr_batch_size //= 2

    if rank == 0:
        print()
        for k in training_set_dict.keys():
            print(k)
            print('Num images: ', len(training_set_dict[k]))
            print('Image shape:', training_set_dict[k].image_shape)
            print('Label shape:', training_set_dict[k].label_shape)
        print()
